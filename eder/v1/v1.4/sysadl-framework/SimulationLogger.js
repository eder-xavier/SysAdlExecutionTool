...
class SimulationLogger {
  ...
  getFormattedTrace(includeBuilding = true, monitoredPorts = []) {
    let output = '';
    const buildingTypes = ['COMPONENT_INSTANTIATION', 'PORT_INSTANTIATION', 'CONNECTION_ESTABLISHED'];

    this.events.forEach(event => {
      if (!includeBuilding && buildingTypes.includes(event.type)) return;
      output += this.formatEvent(event);
      output += '\n';
    });

    output += '\n' + '='.repeat(80) + '\n';
    output += this.getFormattedSummary(includeBuilding, monitoredPorts);
  ...
  getFormattedSummary(includeBuilding = true, monitoredPorts = []) {
    ...
    this.events.forEach(e => {
      if (!includeBuilding && buildingTypes.includes(e.type)) return;
    ...
    // Final port values (Fuzzy matched)
    const finalValues = this.getFinalValues(includeBuilding, monitoredPorts);
  ...
  getFinalValues(includeBuilding = true, monitoredPorts = []) {
    const finalValues = {};
    this.events.forEach(e => {
      if (e.type === 'PORT_RECEIVE' || e.type === 'PORT_SEND') {
        const portName = e.data.portPath;
        if (monitoredPorts && monitoredPorts.length > 0) {
          // Fuzzy match: check if exact match or if it's a suffix/prefix path ending
          if (monitoredPorts.some(mp => mp === portName || mp.endsWith('.' + portName) || portName.endsWith('.' + mp))) {
            finalValues[portName] = e.data.value;
          }
        } else {
          finalValues[portName] = e.data.value;
        }
  ...
  printSummary(includeBuilding = true, monitoredPorts = []) {
