...
function runBrowser(generatedCode, options = {}) {
  const showBuilding = options.showBuilding === true;
  const monitoredPorts = options.monitoredPorts || [];
  ...
  try {
    ...
    if (simulationLogger && typeof model.attachSimulationLogger === 'function') {
      ...
      if (simulationLogger) {
        ...
        const trace = simulationLogger.getFormattedTrace(showBuilding, monitoredPorts);
        output += '\n' + '='.repeat(80) + '\n';
        ...
        simulationLogger.printSummary(showBuilding, monitoredPorts);
      }
