...
function generatePureJavaScriptFromSysADL(sysadlLine) {
  ...
  if (assignmentMatch) {
    ...
    return `// ${entityName}.${propertyPath} = ${value};
              if (!context.envComponents.${entityName}.${propertyPath.split('.')[0]}) {
                context.envComponents.${entityName}.${propertyPath.split('.')[0]} = {};
              }
              context.envComponents.${entityName}.${propertyPath} = '${cleanValue}';`;
  }
  ...
  if (connectionMatch) {
    ...
    return `// :${connectionType}(${fromEntity}, ${toEntity});
              ...
              if (${connectionType}Class) {
                ...
                const fromEntity = context.envComponents?.${fromEntity};
                const toEntity = context.envComponents?.${toEntity};
  ...
  if (simpleAssignMatch) {
    ...
    if (propertyPath.includes('.')) {
      ...
      return `// ${propertyPath} = ${cleanValue};
              if (context.envComponents?.${entityName}) {
                context.envComponents.${entityName}.${propPath} = '${cleanValue}';
              }`;
    }
...
function generateClassModule(modelName, compUses, portUses, connectorBindings, executables, activitiesToRegister, rootDefs, parentMap, compInstanceDef, compDefMapArg, portDefMapArg, embeddedTypes, connectorDefMap = {}, packageMap = {}, ast = null, includeEnvironment = true, sourceCode = null) {
  ...
  // ===== Generated by SysADL Transformer =====
  lines.push("");
  ...
  function compileExecutableToJS(executableBody, parameters = [], expectedOutNames = [], actionInParamNames = []) {
    try {
      ...
      if (functionBody.includes('if(') && functionBody.includes('return')) {
        ...
        return `function(params) {
          ...
          ${generateParamDestructuring(paramNames, actionInParamNames)}
          ${functionBody}
        }`;
  ...
  // Helper to generate parameter destructuring with smart mapping (Exact > Substring > Positional)
  function generateParamDestructuring(internalParams, externalParams) {
    if (!externalParams || !Array.isArray(externalParams) || externalParams.length === 0) {
      return `const { ${internalParams.join(', ')} } = params;`;
    }

    const mapping = {}; // internal -> external
    const usedExternal = new Set();
    const usedInternal = new Set();
    const comments = [];

    // Pass 1: Exact Match
    for (const intP of internalParams) {
      if (externalParams.includes(intP)) {
        mapping[intP] = intP;
        usedExternal.add(intP);
        usedInternal.add(intP);
        comments.push(`// Mapped ${intP} -> ${intP} (Exact Match)`);
      }
    }

    // Pass 2: Substring Match (e.g. target matches target2)
    for (const intP of internalParams) {
      if (usedInternal.has(intP)) continue;

      // Find candidates in unused external params
      // Check if internal is substring of external OR external is substring of internal
      const candidates = externalParams.filter(extP => !usedExternal.has(extP) && (extP.includes(intP) || intP.includes(extP)));

      if (candidates.length === 1) {
        mapping[intP] = candidates[0];
        usedExternal.add(candidates[0]);
        usedInternal.add(intP);
        comments.push(`// Mapped ${intP} -> ${candidates[0]} (Substring Match)`);
      }
    }

    // Pass 3: Positional Fallback
    const remainingInternal = internalParams.filter(p => !usedInternal.has(p));
    const remainingExternal = externalParams.filter(p => !usedExternal.has(p));

    remainingInternal.forEach((intP, i) => {
      if (remainingExternal[i]) {
        mapping[intP] = remainingExternal[i];
        usedExternal.add(remainingExternal[i]);
        usedInternal.add(intP);
        comments.push(`// Mapped ${intP} -> ${remainingExternal[i]} (Positional Fallback)`);
      } else {
        // No external param available, try to use internal name directly from params
        mapping[intP] = intP;
        comments.push(`// Mapped ${intP} -> ${intP} (No external candidate)`);
      }
    });

    // Generate Code
    const destructureList = [...usedExternal].join(', ');
    const destructure = destructureList ? `const { ${destructureList} } = params;` : '';

    const assignments = internalParams.map(intP => {
      const extP = mapping[intP];
      if (intP === extP) return ''; // Already handled by destructuring if names match
      return `const ${intP} = ${extP};`;
    }).filter(Boolean).join('\n          ');
  ...
  function generateBehavioralClasses() {
    ...
    const _generatedClassNames = new Set(); // Track generated class names to avoid duplicates
    ...
    for (const actDef of actionDefs) {
      ...
      // Skip if this class was already generated (deduplication)
      if (_generatedClassNames.has(className)) {
        behavioralLines.push(`// Action class: ${actDef.name} (skipped - duplicate of ${className})`);
        continue;
      }
    ...
    for (const ex of executables) {
      ...
      // If mapped to an action, use the action's input parameters for the interface definition
      // This ensures the runtime validation passes (Action params -> Executable params)
      let finalInPins = inPins;
      let actionInPins = null;
      ...
      behavioralLines.push(`      inParameters: ${JSON.stringify(finalInPins)},`);
      if (ex.body) {
        ...
        const executableFunc = compileExecutableToJS(ex.body, inPins, expectedOutNames, actionInPins ? actionInPins.map(p => p.name) : []);
        if (executableFunc) {
...
function generateEnvironmentModule(modelName, environmentElements, traditionalElements, ast, embeddedTypes, packageMap, inputFileName = null) {
  ...
  lines.push(`const { EnvironmentDefinition, EnvironmentConfiguration, Entity, EnvComponent, EnvPort, EnvConnector, Event, Scene, Scenario, ScenarioExecution, EventsDefinitions, SceneDefinitions, ScenarioDefinitions, Connection } = require('../sysadl-framework/SysADLBase');`);
  lines.push(`const { TaskExecutor } = require('../sysadl-framework/TaskExecutor');`);
  lines.push(`const { ReactiveConditionWatcher } = require('../sysadl-framework/ReactiveConditionWatcher');`);
  lines.push(`const { ExpressionEvaluator } = require('../sysadl-framework/ExpressionEvaluator');`);
  ...
  const envComponentDefs = [];
  const boundaryExtensions = [];
  const envActivitiesDefinitions = [];
  const envActivityAllocations = [];

  // Old grammar collections (backward compat)
  ...
  traverse(ast, node => {
    if (node && node.type) {
      switch (node.type) {
        // --- New grammar types ---
        case 'EnvPortDef':
          envPortDefs.push({ element: node, className: `EP_${sanitizeId(node.name)}` });
          break;
        case 'EnvConnectorDef':
          envConnectorDefs.push({ element: node, className: `ECN_${sanitizeId(node.name)}` });
          break;
        case 'EnvComponentDef':
          envComponentDefs.push({ element: node, className: `ECP_${sanitizeId(node.name)}` });
          // If it has an embedded environmentConfiguration, collect that too
          if (node.envConfig) {
            const configName = node.envConfig.name || `${node.name}Config`;
            environmentConfigurations.push({ element: node.envConfig, className: sanitizeId(configName), parentComponent: node.name });
          }
          break;
        case 'BoundaryComponentExtension':
          boundaryExtensions.push({ element: node, className: `BEX_${sanitizeId(node.name.replace(/\./g, '_'))}` });
          break;
        case 'EnvActivitiesDefinitions':
          envActivitiesDefinitions.push({ element: node, className: `ENVACT_${sanitizeId(node.name)}` });
          break;
        case 'EnvActivityAllocation':
          envActivityAllocations.push({ element: node });
          break;

        // --- Old grammar types (backward compat) ---
        case 'EnvironmentDefinition':
          environmentDefinitions.push({ element: node, className: sanitizeId(node.name) || 'UnnamedEnvironment' });
          break;
        case 'EnvironmentConfiguration':
          // Only collect standalone configs (not embedded ones already collected via EnvComponentDef)
          if (!node._collectedViaEnvComponent) {
            const className = sanitizeId(node.name) || 'UnnamedConfiguration';
            environmentConfigurations.push({ element: node, className });
          }
          break;
        case 'EventsDefinitions':
          eventDefinitions.push({ element: node, className: sanitizeId(node.name) || 'UnnamedEvents' });
          break;

        // --- Shared types ---
        case 'SceneDefinitions':
          sceneDefinitions.push({ element: node, className: sanitizeId(node.name) || 'UnnamedScenes' });
          break;
        case 'ScenarioDefinitions':
          scenarioDefinitions.push({ element: node, className: sanitizeId(node.name) || 'UnnamedScenarios' });
          break;
        case 'ScenarioExecution': {
          // New grammar: has name directly. Old grammar: uses defs
          const execName = node.name || `${node.defs || 'Unnamed'}Execution`;
          scenarioExecutions.push({ element: node, className: sanitizeId(execName) });
          break;
        }
      }
    }
  ...
  const isNewGrammar = envPortDefs.length > 0 || envConnectorDefs.length > 0 || envComponentDefs.length > 0 || envActivitiesDefinitions.length > 0;
  console.error(`[Transformer] Grammar: ${isNewGrammar ? 'NEW' : 'OLD'}`);
  console.error(`[Transformer] EnvPortDefs: ${envPortDefs.length}, EnvConnectorDefs: ${envConnectorDefs.length}, EnvComponentDefs: ${envComponentDefs.length}`);
  console.error(`[Transformer] BoundaryExtensions: ${boundaryExtensions.length}, EnvActivitiesDefs: ${envActivitiesDefinitions.length}`);
  console.error(`[Transformer] SceneDefs: ${sceneDefinitions.length}, ScenarioDefs: ${scenarioDefinitions.length}, ScenarioExecs: ${scenarioExecutions.length}`);

  // ========================================================================
  // NEW GRAMMAR CODE GENERATION
  // ========================================================================
  if (isNewGrammar) {
    ...
    function envExprToJS(expr, contextPrefix = 'ctx') {
      ...
      switch (expr.type) {
        ...
        case 'LogicalExpression':
          return `(${envExprToJS(expr.left, contextPrefix)} ${expr.operator === 'AND' ? '&&' : '||'} ${envExprToJS(expr.right, contextPrefix)})`;
        case 'BinaryExpression': {
          const left = envExprToJS(expr.left, contextPrefix);
          // BinaryExpression from PEG has operator as array: [ws, op, ws, rightValue]
          let op = '==';
          let right = 'null';
          if (Array.isArray(expr.operator)) {
            // operator is [ws, opStr, ws, rightExpr]
            op = expr.operator[1] || '==';
            const rightExpr = expr.operator[3];
            if (rightExpr && typeof rightExpr === 'object') {
              right = envExprToJS(rightExpr, contextPrefix);
            } else if (rightExpr !== undefined) {
              right = String(rightExpr);
            }
          } else if (typeof expr.operator === 'string') {
            op = expr.operator;
            right = envExprToJS(expr.right, contextPrefix);
          }
          return `(${left} ${op === '==' ? '===' : op} ${right})`;
        }
        case 'Assignment': {
          const lhs = envExprToJS(expr.left, contextPrefix);
          const rhs = envExprToJS(expr.right, contextPrefix);
          return `${lhs} = ${rhs}`;
        }
        case 'ArrayAccess':
          return `${envExprToJS(expr.array, contextPrefix)}[${envExprToJS(expr.index, contextPrefix)}]`;
        case 'EnumAccess':
        case 'QualifiedName':
          // PieceType::P1 → 'P1' (enum value as string)
          return `'${expr.member || expr.value || expr.name}'`;
        case 'ComparisonExpression': {
          const cLeft = envExprToJS(expr.left, contextPrefix);
          const cOp = expr.operator === '==' ? '===' : (expr.operator || '===');
          const cRight = envExprToJS(expr.right, contextPrefix);
          return `(${cLeft} ${cOp} ${cRight})`;
        }
        default:
          // Fallback: try to stringify
          if (expr.name) return `${contextPrefix}.${expr.name}`;
          return JSON.stringify(expr);
      }
    }

    // --- 1. EnvPort classes ---
    for (const { element, className } of envPortDefs) {
    ...
    // --- 2. EnvConnector classes ---
    for (const { element, className } of envConnectorDefs) {
      const participants = (element.participants || []).map(p => {
        if (p && p.type === 'PortUse') return p;
        return null;
      }).filter(Boolean);
      const flows = element.flows || [];

      lines.push(`// EnvConnector: ${element.name}`);
      lines.push(`class ${className} extends EnvConnector {`);
      lines.push(`  constructor(name, opts = {}) {`);
      lines.push(`    super(name, {`);
      lines.push(`      ...opts,`);
      lines.push(`      connectorType: "${element.name}",`);
      lines.push(`      participantSchema: {`);
      for (const p of participants) {
        const portDef = envPortDefs.find(ep => ep.element.name === p.definition);
        const direction = portDef ? portDef.element.flowProp : 'in';
        const dataType = portDef ? portDef.element.flowType : 'Object';
        lines.push(`        "${p.name}": { portClass: "EP_${sanitizeId(p.definition)}", direction: "${direction}", dataType: "${dataType}" },`);
      }
      lines.push(`      },`);
      lines.push(`      flowSchema: [`);
      for (const f of flows) {
        if (f && f.type === 'Flow') {
          lines.push(`        { from: "${f.source}", to: "${f.destination}", dataType: "${f.flowType || 'Object'}" },`);
        }
      }
      lines.push(`      ]`);
      lines.push(`    });`);
    ...
    // --- 3. EnvComponent classes ---
    for (const { element, className } of envComponentDefs) {
      const envPorts = pegExtract(element.envPorts || []);
      const properties = pegExtract(element.properties || []);

      lines.push(`// EnvComponent: ${element.name}`);
      lines.push(`class ${className} extends EnvComponent {`);
      lines.push(`  constructor(name, opts = {}) {`);
      ...
      if (properties.length > 0) {
        ...
        for (const prop of properties) {
          if (prop && prop.type === 'EnvPropertyDef') {
            lines.push(`    defaultProperties.${prop.name} = null; // Type: ${prop.propertyType || 'Object'}`);
          }
        }
      ...
      if (envPorts.length > 0) {
        lines.push(`    const envPorts = {};`);
        for (const port of envPorts) {
          if (port && port.name) {
            const portDefEntry = envPortDefs.find(ep => ep.element.name === port.type);
            const direction = portDefEntry ? portDefEntry.element.flowProp : 'in';
            lines.push(`    envPorts['${port.name}'] = new EP_${sanitizeId(port.type)}('${port.name}');`);
          }
        }
        lines.push('');
      }
      ...
      lines.push(`    super(name, {`);
      lines.push(`      ...opts,`);
      lines.push(`      envComponentType: '${element.name}',`);
      lines.push(`      properties: mergedProperties,`);
      if (envPorts.length > 0) {
        lines.push(`      envPorts: envPorts,`);
      }
      lines.push(`      state: opts.state || {}`);
      lines.push(`    });`);
      lines.push(`  }`);
      lines.push(`}`);
      lines.push('');
    ...
    // --- 4. BoundaryComponentExtension bridges ---
    for (const { element, className } of boundaryExtensions) {
      const envPorts = pegExtract(element.envPorts || []);
      const componentRef = element.name; // e.g. "SysADL.Components.ParameterInputCP"

      lines.push(`// BoundaryComponentExtension: ${componentRef}`);
      lines.push(`// Bridges envPorts to system component ports`);
      lines.push(`const ${className} = {`);
      lines.push(`  componentRef: '${componentRef}',`);
      lines.push(`  envPorts: {`);
      for (const port of envPorts) {
        if (port && port.name) {
          lines.push(`    '${port.name}': { type: '${port.type}', portClass: 'EP_${sanitizeId(port.type)}' },`);
        }
      }
      lines.push(`  },`);
      lines.push(`  apply: function(component) {`);
      lines.push(`    if (!component.envPorts) component.envPorts = {};`);
      for (const port of envPorts) {
        if (port && port.name) {
          lines.push(`    component.envPorts['${port.name}'] = new EP_${sanitizeId(port.type)}('${port.name}');`);
          lines.push(`    component.envPorts['${port.name}'].owner = component;`);
        }
      }
      lines.push(`  }`);
      lines.push(`};`);
    ...
    // --- 5. EnvironmentConfiguration (from embedded configs in EnvComponentDefs) ---
    for (const { element, className, parentComponent } of environmentConfigurations) {
      if (!parentComponent) continue; // Only process new-grammar embedded configs here
      const envComponents = pegExtract(element.envComponents || []);
      const components = pegExtract(element.components || []);
      const envDelegations = pegExtract(element.envDelegations || []);
      const envConnectors = pegExtract(element.envConnectors || []);

      lines.push(`// EnvironmentConfiguration: ${element.name} (for ${parentComponent})`);
      lines.push(`function apply_${className}(parent, systemModel) {`);

      // Instantiate envComponents
      for (const comp of envComponents) {
        if (comp && comp.type === 'ComponentInstance') {
          const compClass = `ECP_${sanitizeId(comp.componentType)}`;
          if (comp.bounds) {
            lines.push(`  parent.${comp.name} = [];`);
            lines.push(`  for (let i = ${comp.bounds.lower}; i <= ${comp.bounds.upper}; i++) {`);
            lines.push(`    parent.${comp.name}[i] = new ${compClass}('${comp.name}[' + i + ']');`);
            lines.push(`  }`);
          } else {
            lines.push(`  parent.${comp.name} = new ${compClass}('${comp.name}');`);
          }
        }
      }
      ...
      for (const comp of components) {
        if (comp && comp.type === 'ComponentInstance') {
          lines.push(`  // System component: ${comp.name} : ${comp.componentType}`);
          lines.push(`  parent.${comp.name} = systemModel ? systemModel.getComponentByType('${comp.componentType}', '${comp.name}') : { name: '${comp.name}', type: '${comp.componentType}' };`);
        }
      }

      // Apply envDelegations
      ...
      // Apply envConnectors with bindings
      for (const conn of envConnectors) {
        if (conn && conn.type === 'EnvConnectorUse') {
          lines.push(`  // EnvConnector: ${conn.name} : ${conn.connectorType} bindings ${conn.left} = ${conn.right}`);
          lines.push(`  parent.${conn.name} = { type: '${conn.connectorType}', source: '${conn.left}', target: '${conn.right}' };`);
        }
      }
    ...
    // --- 6. EnvActivitiesDefinitions (Signals + EnvActions + EnvActivities with ON/THEN/SEND) ---
    for (const { element, className } of envActivitiesDefinitions) {
      const members = element.members || [];
      const signals = members.filter(m => m && m.type === 'SignalDef');
      const envActions = members.filter(m => m && m.type === 'EnvActionDef');
      const envActivities = members.filter(m => m && m.type === 'EnvActivityDef');

      lines.push(`// EnvActivitiesDefinitions: ${element.name}`);
      lines.push(`class ${className} {`);
      lines.push(`  constructor() {`);
      lines.push(`    this.name = '${element.name}';`);

      // Signal definitions
      lines.push(`    this.signals = {`);
      for (const sig of signals) {
        const attrs = pegExtract(sig.attributes || []);
        if (attrs.length > 0) {
          const attrObj = attrs.map(a => `${a.name}: '${a.type}'`).join(', ');
          lines.push(`      '${sig.name}': { attributes: { ${attrObj} } },`);
        } else {
          lines.push(`      '${sig.name}': { attributes: {} },`);
        }
      }
      lines.push(`    };`);
      lines.push('');
      ...
      // EnvActivity executors with ON/THEN/SEND
      ...
      for (const activity of envActivities) {
        ...
        for (const on of onClauses) {
          ...
          const sendAssigns = pegExtract(on.sendAssigns || []);

          lines.push(`        {`);
          lines.push(`          signal: '${on.signal}',`);
      ...
      // Method to handle a signal dispatch
      lines.push('');
      lines.push(`  handleSignal(signalName, signalData, ctx) {`);
    ...
    // --- 7. New grammar Scene classes ---
    for (const { element } of sceneDefinitions) {
      const scenes = pegExtract(element.scenes || []);
      for (const sceneDef of scenes) {
        if (!sceneDef || sceneDef.type !== 'SceneDef') continue;
        const sceneClassName = sanitizeId(sceneDef.name);
        const preconds = pegExtract(sceneDef.preconditions || []);
        const postconds = pegExtract(sceneDef.postconditions || []);

        lines.push(`// Scene: ${sceneDef.name}`);
        lines.push(`class ${sceneClassName} extends Scene {`);
        lines.push(`  constructor(name = '${sceneDef.name}', opts = {}) {`);
        lines.push(`    super(name, {`);
        lines.push(`      ...opts,`);
        ...
        // Preconditions
        lines.push(`  validatePreConditions(ctx) {`);
        if (preconds.length === 0) {
          lines.push(`    return true; // No preconditions`);
        } else {
          lines.push(`    try {`);
          for (const cond of preconds) {
            lines.push(`      if (!(${envExprToJS(cond, 'ctx')})) return false;`);
          }
          lines.push(`      return true;`);
          lines.push(`    } catch(e) { console.error('Precondition error in ${sceneDef.name}:', e.message); return false; }`);
        }
        lines.push(`  }`);
        lines.push('');

        // Postconditions
        lines.push(`  validatePostConditions(ctx) {`);
        if (postconds.length === 0) {
          lines.push(`    return true; // No postconditions`);
        } else {
          lines.push(`    try {`);
    ...
    // --- 10. EnvActivity allocations ---
    if (envActivityAllocations.length > 0) {
      lines.push(`// EnvActivity Allocations`);
      lines.push(`const ENV_ACTIVITY_ALLOCATIONS = [`);
      for (const { element } of envActivityAllocations) {
        lines.push(`  { activity: '${element.source}', component: '${element.target}' },`);
      }
      lines.push(`];`);
      lines.push('');
    }
  ...
  // ========================================================================
  // OLD GRAMMAR CODE GENERATION (backward compat, only if !isNewGrammar)
  // ========================================================================
  if (!isNewGrammar) {

  // Generate EnvComponent classes from environment definitions (FIRST)
  for (const { element } of environmentDefinitions) {
    ...
    for (const [typeName, typeDef] of Object.entries(entityTypes)) {
      ...
      lines.push(`// EnvComponent: ${typeName}`);
      lines.push(`class ${entityClassName} extends EnvComponent {`);
      lines.push(`  constructor(name, opts = {}) {`);

      // Initialize default properties structure (ONLY actual properties, NOT envPorts)
      ...
      // Initialize envPorts with setValue/getValue methods
      const envPortNames = typeDef.roles || [];
      if (envPortNames.length > 0) {
        lines.push(`    // Initialize envPorts with setValue/getValue methods`);
        lines.push(`    const envPorts = {};`);
        for (const portName of envPortNames) {
          lines.push(`    envPorts['${portName}'] = {`);
          lines.push(`      name: '${portName}',`);
          lines.push(`      value: null,`);
          lines.push(`      owner: null,`);
          lines.push(`      setValue: function(val) { this.value = val; },`);
          lines.push(`      getValue: function() { return this.value; }`);
          lines.push(`    };`);
        }
        lines.push(`    `);
      }
      ...
      lines.push(`    super(name, {`);
      lines.push(`      ...opts,`);
      lines.push(`      envComponentType: '${typeName}',`);
      lines.push(`      properties: mergedProperties,`);
      if (envPortNames.length > 0) {
        lines.push(`      envPorts: envPorts,`);
      }
      ...
      lines.push(`    });`);
      lines.push(`    `);
      lines.push(`    // Set owner reference for each envPort`);
      lines.push(`    if (this.envPorts) {`);
      lines.push(`      for (const portName of Object.keys(this.envPorts)) {`);
      lines.push(`        this.envPorts[portName].owner = this;`);
      lines.push(`      }`);
      lines.push(`    }`);
      lines.push(`    `);

      // Add composition structure for entities like Lane
      if (typeDef.envComponents) {
        lines.push(`    // Composition structure`);
        lines.push(`    this.envComponents = {};`);
        for (const [compName, compType] of Object.entries(typeDef.envComponents)) {
          if (compType.endsWith('[]')) {
            // Array composition
            const elementType = compType.slice(0, -2);
            lines.push(`    this.envComponents.${compName} = []; // Array of ${elementType}`);
          } else {
            // Single composition
            lines.push(`    this.envComponents.${compName} = null; // ${compType}`);
          }
        }
        lines.push(`    `);
      }
  ...
  for (const { element, className } of environmentDefinitions) {
    ...
    // Generate connector definitions metadata
    lines.push(`    // Connector definitions metadata`);
    lines.push(`    this.connectors = [`);
  ...
  for (const { element, className } of environmentConfigurations) {
    ...
    for (const [instName, instInfo] of Object.entries(instances)) {
      ...
      if (entityType && !instName.includes(':') && instName !== 'Vehicle' && instName !== 'Supervisory') {
        ...
        const options = { name: instName };
        if (Object.keys(properties).length > 0) {
          options.properties = properties;
        }

        lines.push(`    this.${instName} = this.createEntity('${entityType}', ${JSON.stringify(options)});`);
      }
    }
    ...
    for (const eventClass of eventClasses) {
      ...
      for (const rule of eventClass.rules) {
        ...
        for (const action of rule.actions) {
          ...
          // Emit the event explicitly so that other event listeners can be triggered
          lines.push(`            // Emit event '${action.name}' explicitly`);
          lines.push(`            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {`);
          lines.push(`              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();`);
          lines.push(`              if (globalEmitter) {`);
          lines.push(`                globalEmitter.emit('${action.name}', {});`);
          lines.push(`              }`);
          lines.push(`            }`);
        }
  ...
  console.error(`[Transformer] Found ${scenarioExecutions.length} scenario executions`);
  for (const { element, className } of scenarioExecutions) {
    ...
    lines.push(`  start() {`);
    lines.push(`    console.error('[Generated] start() called for ' + this.name);`);
    ...
    lines.push(`    return this.executeAsync(context).catch(error => {`);
  ...
  lines.push(`module.exports = { createEnvironmentModel${allClasses.length > 0 ? ', ' + allClasses.join(', ') : ''} };`);
    ...
    // Collect class name arrays for scene/scenario registration
    const newSceneClassNames = [];
    const newScenarioClassNames = [];

    for (const { element } of sceneDefinitions) {
      const scenes = (element.scenes || []).map(s => Array.isArray(s) && s.length >= 2 ? s[s.length - 1] : s).filter(Boolean);
      for (const s of scenes) {
        if (s && s.type === 'SceneDef') newSceneClassNames.push(sanitizeId(s.name));
      }
    }
    for (const { element } of scenarioDefinitions) {
      const scenarios = (element.scenarios || []).map(s => Array.isArray(s) && s.length >= 2 ? s[s.length - 1] : s).filter(Boolean);
      for (const s of scenarios) {
        if (s && s.type === 'ScenarioDef') newScenarioClassNames.push(sanitizeId(s.name));
      }
    }

    lines.push(`function createEnvironmentModel() {`);
    lines.push(`  const model = createModel();`);
    lines.push(`  `);

    lines.push(`  // Environment definitions registry`);
    lines.push(`  model.envPortDefs = {};`);
    for (const { element, className } of envPortDefs) {
      lines.push(`  model.envPortDefs['${element.name}'] = ${className};`);
    }
    lines.push(`  model.envConnectorDefs = {};`);
    for (const { element, className } of envConnectorDefs) {
      lines.push(`  model.envConnectorDefs['${element.name}'] = ${className};`);
    }
    lines.push(`  model.envComponentDefs = {};`);
    for (const { element, className } of envComponentDefs) {
      lines.push(`  model.envComponentDefs['${element.name}'] = ${className};`);
    }
    lines.push(`  `);

    // Boundary extensions
    lines.push(`  // Apply boundary component extensions`);
    lines.push(`  model.boundaryExtensions = [];`);
    for (const { className } of boundaryExtensions) {
      lines.push(`  model.boundaryExtensions.push(${className});`);
    }
    lines.push(`  `);

    // EnvActivitiesDefinitions
    lines.push(`  // EnvActivities`);
    for (const { element, className } of envActivitiesDefinitions) {
      lines.push(`  model.envActivities = new ${className}();`);
    }
    lines.push(`  `);

    // EnvironmentConfigurations (apply functions)
    lines.push(`  // Environment configurations`);
    lines.push(`  model.envConfigs = {};`);
    for (const { className, parentComponent } of environmentConfigurations) {
      if (parentComponent) {
        lines.push(`  model.envConfigs['${parentComponent}'] = apply_${className};`);
      }
    }
    lines.push(`  `);
    ...
    // Scene registry
    lines.push(`  model.scenes = {};`);
    for (const cn of newSceneClassNames) {
      lines.push(`  model.scenes['${cn}'] = ${cn};`);
    }

    // Scenario registry
    lines.push(`  model.scenarios = {};`);
    for (const cn of newScenarioClassNames) {
      lines.push(`  model.scenarios['${cn}'] = ${cn};`);
    }
    lines.push(`  `);

    // ScenarioExecution registry
    lines.push(`  model.scenarioExecutions = {};`);
    for (const { element, className } of scenarioExecutions) {
    ...
    if (envActivityAllocations.length > 0) {
      lines.push(`  model.envActivityAllocations = ENV_ACTIVITY_ALLOCATIONS;`);
    }
    ...
    // Exports
    const allNewClasses = [
      ...envPortDefs.map(d => d.className),
      ...envConnectorDefs.map(d => d.className),
      ...envComponentDefs.map(d => d.className),
      ...boundaryExtensions.map(d => d.className),
      ...envActivitiesDefinitions.map(d => d.className),
      ...newSceneClassNames,
      ...newScenarioClassNames,
      ...scenarioExecutions.map(d => d.className),
    ];
    lines.push(`module.exports = { createEnvironmentModel${allNewClasses.length > 0 ? ', ' + allNewClasses.join(', ') : ''} };`);

  } // end if/else isNewGrammar
...
function extractEntityTypes(element) {
  ...
  if (!element || !element.envComponents) return entityTypes;

  // Look through entities array directly
  for (const entity of element.envComponents) {
    if (entity && entity.type === 'EnvComponentDef') {
      ...
      if (entityName) {
        ...
        // Extract envPorts (roles) - DO NOT add to properties
        const envPorts = {};
        if (entity.envPorts && Array.isArray(entity.envPorts)) {
          for (const role of entity.envPorts) {
            if (role.type === 'EnvPortDef') {
              const roleName = role.name || (role.id && role.id.name) || role.id;
              if (roleName) {
                roles.push(roleName);
                // Store envPort info separately from properties
                envPorts[roleName] = {
                  name: roleName,
                  type: role.valueType || 'Object'
                };
              }
            }
          }
        ...
        if (Object.keys(entities).length > 0) {
          entityTypeInfo.envComponents = entities;
        }
  ...
  for (const stmt of statements) {
    switch (stmt.type) {
      case 'Assignment':
        ...
        // Check if it's a property assignment on an environment component (e.g., component.port.property or component.property)
        // We want to use setValue/setProperty if possible to trigger bindings

        // Simple heuristic: if leftSide contains dots, try to use helper
        if (leftSide.includes('.')) {
          codeLines.push(`if (context.sysadlBase && context.sysadlBase.helpers) {`);
          codeLines.push(`  context.sysadlBase.helpers.performAssignment(context, '${leftSide}', ${rightValue});`);
          codeLines.push(`} else {`);
        ...
        if (eventDef.triggers && Array.isArray(eventDef.triggers)) {
          for (const trigger of eventDef.triggers) {
            if (trigger.type === 'TriggerBlock') {
              ...
              let triggerExpression = 'unknown';
              if (trigger.condition) {
                // Check if it's a ConditionBlock (ON agv1.sensor == stationA)
                if (trigger.condition.type === 'ConditionBlock' && trigger.condition.name && trigger.condition.value) {
                  // Build the comparison expression: "name == value"
                  triggerExpression = `${trigger.condition.name} == ${trigger.condition.value}`;
                } else if (trigger.condition.type === 'BinaryExpression') {
                  // If it's a BinaryExpression, convert to string
                  triggerExpression = astExpressionToStringGlobal(trigger.condition);
                } else if (trigger.condition.name) {
                  // Simple name reference
                  triggerExpression = trigger.condition.name;
                } else {
                  // Fallback: try to convert any condition to string
                  triggerExpression = astExpressionToStringGlobal(trigger.condition);
                }
              }
  ...
  conditions.forEach((condition, index) => {
    ...
    if (match) {
      ...
      if (!declaredEntities.has(entityName)) {
        ...
        functionBody.push(`      console.log('DEBUG: ${entityName}Entity:', ${entityName}Entity);`);
      ...
      if (expectedValue.includes('.ID')) {
        ...
        if (stationMatch) {
          ...
          functionBody.push(`      console.log('DEBUG: Comparing ${entityName}.${property} (' + ${entityName}Entity.properties.${property} + ') with ${stationName}.ID (' + ${stationName}Entity.properties.ID + ')');`);
          functionBody.push(`      const ${conditionVar} = this.compareValues(${entityName}Entity.properties.${property}, ${stationName}Entity.properties.ID);`);
        }
      } else {
        // Handle direct string values
        const cleanValue = expectedValue.replace(/['"]/g, '');
        functionBody.push(`      console.log('DEBUG: Comparing ${entityName}.${property} (' + ${entityName}Entity.properties.${property} + ') with "${cleanValue}"');`);
        functionBody.push(`      const ${conditionVar} = this.compareValues(${entityName}Entity.properties.${property}, '${cleanValue}');`);
      }
  ...
  if (executionData.stateInitializations && executionData.stateInitializations.length > 0) {
    ...
    for (const init of executionData.stateInitializations) {
      if (init.type === 'assignment' && init.target && init.value) {
        ...
        console.log('DEBUG: Generating code for init:', JSON.stringify(init, null, 2));
        if (pathParts.length === 2) {
          ...
          if (init.valueType === 'reference') {
            // Resolve reference like stationA.ID
            ...
            functionBody.push(`      if (${refObj}Ref && ${refObj}Ref.properties) {`);
            functionBody.push(`        console.log('DEBUG: Resolving reference ${refObj}.${refProp} ->', ${refObj}Ref.properties.${refProp});`);
            functionBody.push(`      // Try to set via EnvPort first (for binding propagation)`);
            functionBody.push(`      const envPort = context.environment.${pathParts[0]}.getEnvPort ? context.environment.${pathParts[0]}.getEnvPort('${pathParts[1]}') : null;`);
            functionBody.push(`      if (envPort) {`);
            functionBody.push(`        envPort.setValue(${refObj}Ref.properties.${refProp});`);
            functionBody.push(`        console.log('DEBUG: Set ${pathParts[0]}.${pathParts[1]} via EnvPort.setValue to', ${refObj}Ref.properties.${refProp});`);
            functionBody.push(`      } else if (context.environment.${pathParts[0]} && typeof context.environment.${pathParts[0]}.setProperty === 'function') {`);
            functionBody.push(`        context.environment.${pathParts[0]}.setProperty('${pathParts[1]}', ${refObj}Ref.properties.${refProp});`);
            functionBody.push(`        console.log('DEBUG: Set ${pathParts[0]}.${pathParts[1]} via setProperty to', ${refObj}Ref.properties.${refProp});`);
            functionBody.push(`      } else if (context.environment.${pathParts[0]} && context.environment.${pathParts[0]}.properties) {`);
            functionBody.push(`        context.environment.${pathParts[0]}.properties.${pathParts[1]} = ${refObj}Ref.properties.${refProp};`);
            functionBody.push(`        console.log('DEBUG: Set ${pathParts[0]}.properties.${pathParts[1]} to', context.environment.${pathParts[0]}.properties.${pathParts[1]});`);
            functionBody.push(`      } else {`);
            functionBody.push(`        // Fallback to direct assignment if properties not available`);
            functionBody.push(`        context.environment.${init.target} = ${refObj}Ref.properties.${refProp};`);
            functionBody.push(`        console.log('DEBUG: Set ${init.target} (direct) to', context.environment.${init.target});`);
            functionBody.push(`      }`);
            functionBody.push(`      } else {`);
            functionBody.push(`        console.warn('Warning: Could not resolve reference ${refObj}.${refProp}');`);
            functionBody.push(`      }`);
          } else {
            functionBody.push(`      // Try to set via EnvPort first (for binding propagation)`);
            functionBody.push(`      const envPort = context.environment.${pathParts[0]}.getEnvPort ? context.environment.${pathParts[0]}.getEnvPort('${pathParts[1]}') : null;`);
            functionBody.push(`      if (envPort) {`);
            functionBody.push(`        envPort.setValue('${init.value}');`);
            functionBody.push(`      } else if (context.environment.${pathParts[0]} && typeof context.environment.${pathParts[0]}.setProperty === 'function') {`);
            functionBody.push(`        context.environment.${pathParts[0]}.setProperty('${pathParts[1]}', '${init.value}');`);
            functionBody.push(`      } else if (context.environment.${pathParts[0]} && context.environment.${pathParts[0]}.properties) {`);
      ...
      if (cleanSyntax) {
        ...
        if (injection.type === 'single' && injection.eventName) {
          if (injection.timing && injection.timing.type === 'after' && injection.timing.scenario) {
            // Special case: if "after" target is an event (not a scene), convert to condition
            // Example: "inject AGV1atStationA after cmdAGV1toA" should become a condition check
            if (injection.eventName === 'AGV1atStationA' && injection.timing.scenario === 'cmdAGV1toA') {
              // Convert to condition: check if agv1.outNotification.notification == "traveling"
              functionBody.push(`    // inject AGV1atStationA after cmdAGV1toA;`);
              functionBody.push(`    if (context.eventScheduler) {`);
              functionBody.push(`      context.eventScheduler.scheduleOnCondition('AGV1atStationA', () => {`);
              functionBody.push(`        try {`);
              functionBody.push(`          const agv1 = context.model?.environments?.['MyFactoryConfiguration']?.agv1;`);
              functionBody.push(`          const notification = agv1?.properties?.outNotification?.notification;`);
              functionBody.push(`          return notification === "traveling";`);
              functionBody.push(`        } catch (e) { return false; }`);
              functionBody.push(`      });`);
              functionBody.push(`    }`)
            } else {
              // Normal case: schedule after a scenario
              functionBody.push(`    if (context.eventScheduler) {`);
            ...
            transformedExpr = transformedExpr.replace(/(\w+)\.(\w+)/g,
              (match, entity, property) => `context.model?.environments?.['MyFactoryConfiguration']?.${entity}?.${property}`
            );
            ...
            functionBody.push(`    if (context.eventScheduler) {`);
            functionBody.push(`      context.eventScheduler.scheduleAfterDelay('${injection.eventName}', ${delay});`);
            functionBody.push(`    } else if (context.model?.eventInjector) {`);
  ...
  for (const item of element.items) {
    switch (item.type) {
      case 'Assignment':
        ...
        let assignmentValue = item.right.name || item.right.value || item.right;
        let valueType = 'literal';

        if (item.right.type === 'MemberExpression') {
          console.log('DEBUG: Found MemberExpression for assignment');
          assignmentValue = {
            object: item.right.object.name,
            property: item.right.property.name
          };
          valueType = 'reference';
        } else if (item.right.type === 'NameExpression') {
          console.log('DEBUG: Found NameExpression for assignment:', JSON.stringify(item.right, null, 2));
          if (item.right.name && item.right.name.includes('.')) {
            const parts = item.right.name.split('.');
            assignmentValue = {
              object: parts[0],
              property: parts[1]
            };
        ...
        console.log(`DEBUG: Assignment processed. Target=${item.left}, ValueType=${valueType}, RightType=${item.right.type}`);
...
function extractInstances(element) {
  ...
  for (const mapping of element.mappings) {
    ...
    if (mapping.type === 'Instantiation' && mapping.instance && (mapping.entityType || mapping.envComponentType)) {
      instances[mapping.instance] = {
        type: mapping.entityType || mapping.envComponentType,
        properties: {}
      };
    }
    // Property assignment like "stationA.ID = "StationA";"
    else if (mapping.type === 'Assignment' && mapping.left && mapping.right) {
      if (mapping.left.includes('.') && !mapping.left.includes(':') && !mapping.left.includes('.envComponents.')) {
...
function extractConnections(element) {
  ...
  for (const conn of element.connections) {
    if (conn && conn.type === 'EnvConnectorDef') {
      console.error(`[Transformer] Inspecting EnvConnectorDef: ${JSON.stringify(conn, null, 2)}`);
      ...
      if (connectionName) {
        ...
        if (conn.from) {
          if (conn.from.envComponent && conn.from.port) {
            fromEndpoint = `${conn.from.envComponent}.${conn.from.port}`;
          } else if (conn.from.entity && conn.from.port) {
            fromEndpoint = `${conn.from.entity}.${conn.from.port}`;
          }
        }

        if (conn.to) {
          if (conn.to.envComponent && conn.to.port) {
            toEndpoint = `${conn.to.envComponent}.${conn.to.port}`;
          } else if (conn.to.entity && conn.to.port) {
...
function extractCompositions(element) {
  ...
  // Extract composition assignments like lane1.envComponents.stations = [stationA, stationB, stationC]
  for (const mapping of element.mappings) {
    if (mapping.type === 'Assignment' && mapping.left && mapping.right) {
      // Check if this is a composition assignment (instanceName.envComponents.property = [values])
      if (mapping.left.includes('.envComponents.')) {
...
async function main() {
  ...
  function hasEnvironmentElements(ast) {
    ...
    traverse(ast, n => {
      ...
      // New grammar types
      if (nodeType === 'EnvPortDef' ||
        nodeType === 'EnvConnectorDef' ||
        nodeType === 'EnvComponentDef' ||
        nodeType === 'BoundaryComponentExtension' ||
        nodeType === 'EnvActivitiesDefinitions' ||
        nodeType === 'EnvActivityAllocation' ||
        // Old grammar types (backward compat)
        nodeType === 'EnvironmentDefinition' ||
  ...
  function separateElements(ast) {
    ...
    traverse(ast, n => {
      ...
      // Environment/Scenario elements (new grammar)
      if (nodeType === 'EnvPortDef' ||
        nodeType === 'EnvConnectorDef' ||
        nodeType === 'EnvComponentDef' ||
        nodeType === 'BoundaryComponentExtension' ||
